...
def main():
    ...
    # Make the plot
    plt.bar(br1, clip_length, color='r', width=barWidth, edgecolor='grey', label='Clip Length')
    plt.bar(br2, compute_time, color='g', width=barWidth, edgecolor='grey', label='Process Time')
    ...
    plt.ylabel('Clip Length/Process Time (sec)', fontweight='bold', fontsize=15)
    plt.xticks([r + barWidth for r in range(len(serial))], zip(record_date, record_time))

    plt.legend()
    plt.title("Change in Distribution of Clip Length and Its processing time: Soumyajit")
    # plt.show()
